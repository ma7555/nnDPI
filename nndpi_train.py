...
import numpy as np
from tensorflow.keras.layers import Dense, Dropout, Embedding, LSTM, Conv1D, MaxPooling1D, Bidirectional, GRU, Input, BatchNormalization
...
def create_model(max_len, dropout_rate=0.05):
    ...
    x = Conv1D(filters=64, kernel_size=25, strides=1, 
                padding='valid', activation='relu')(x)
    x = BatchNormalization()(x)
    x = MaxPooling1D(pool_size=4)(x)
    # x = Dropout(dropout_rate)(x)
    
    x = Conv1D(filters=128, kernel_size=16, strides=1, 
                padding='valid', activation='relu')(x)
    x = BatchNormalization()(x)
    ...
    x = Conv1D(filters=512, kernel_size=2, strides=1, 
                padding='valid', activation='relu')(x)
    x = BatchNormalization()(x)
    x = MaxPooling1D(pool_size=2)(x)
    # x = Dropout(dropout_rate)(x)
    
    x = Bidirectional(GRU(units=64, dropout=dropout_rate, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--multi_gpu', default=True, type=bool, 
                        help='Train on multi gpu system'
                        )
    parser.add_argument('--batch_size', default=3072, type=np.uint32, 
                        help='Training batch_size'
                        )
    parser.add_argument('--max_len', default=1500, type=np.uint32, 
                        help='Preprocessed packet length in bytes'
                        )

    parser.add_argument('--feather_df', default='./CombinedPackets/allpkts.feather', type=str,
                        help='Path to combined packets feather dataframe')
    args = parser.parse_args()

    print('Reading preprocessed packets..')
    allpkts = pd.read_feather(args.feather_df, 
                                columns=[str(i) for i in range(1500)])
    meta = pd.read_feather(args.feather_df, 
                            columns=['tag_1', 'tag_2', 
                                    'tag_3', 'tag_4', 
                                    'protocol', 'filename', 
                                    'ix']
    ...
    le_1, le_2, le_3, le_4 = LabelEncoder(), LabelEncoder(), LabelEncoder(), LabelEncoder()
    ...
    test_index, val_index = next(sss.split(allpkts.iloc[test_valid_index],
    ...
    tag_1_class_weights = class_weight.compute_class_weight('balanced', np.unique(tag_1), tag_1)
    tag_2_class_weights = class_weight.compute_class_weight('balanced', np.unique(tag_2), tag_2)
    tag_3_class_weights = class_weight.compute_class_weight('balanced', np.unique(tag_3), tag_3)
    tag_4_class_weights = class_weight.compute_class_weight('balanced', np.unique(tag_4), tag_4)
    ...
    es = callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=4,
                                verbose=1, mode='min', baseline=None, restore_best_weights=True)

    cp = callbacks.ModelCheckpoint('dpi.h5', monitor = 'val_loss', 
    ...
    model.fit(
                x=allpkts.iloc[train_index].values, 
                y={
