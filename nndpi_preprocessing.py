...
def get_tags(file):
    ...
    if 'vpn' in file:
        ...
        vpn = 'non_vpn'
        file_ = file
    if fnmatch.fnmatch(file_, 'AIMchat*.pca*') or fnmatch.fnmatch(file_, 'aim_chat*.pca*'):
    ...
    elif fnmatch.fnmatch(file_, 'facebookchat*.pca*') or fnmatch.fnmatch(file_, 'facebook_chat*.pca*'):
    ...
    elif fnmatch.fnmatch(file_, 'hangouts_chat*.pca*') or fnmatch.fnmatch(file_, 'hangout_chat*.pca*'):
    ...
    elif fnmatch.fnmatch(file_, 'ICQchat*.pca*') or fnmatch.fnmatch(file_, 'icq_chat*.pca*'):
    ...
    elif fnmatch.fnmatch(file_, 'Torrent*.pca*') or fnmatch.fnmatch(file_, 'bittorrent*.pca*'):
        ...
        raise ValueError(file, 'file with unproper labelling, please rename the file to fit one of the working tags')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pcap_dir', default='./CompletePCAPs/', 
                        type=str, help='Raw PCAP files dir'
                        )

    parser.add_argument(
                        '--processed_pcap_dir', default='./ProcessedPackets/', 
                        type=str, help='Processed PCAP files dir'
                        )
    ...
    pkt_files = [
                 os.path.join(pcap_dir, file) for file in os.listdir(pcap_dir) 
                 if (file.endswith('.pcapng') or file.endswith('.pcap'))
                ]

    processed_pkt_files = [
                           os.path.join(processed_pcap_dir, file) for file 
                           in os.listdir(processed_pcap_dir) 
                           if (file.endswith('.feather'))
                           ]
    to_process_pkts = [
                       pkt for pkt in pkt_files if pkt.split('/')[-1]
                       not in [
    ...
    if args.n_jobs == 1:
        ...
        for filename in pbar:
            ...
            allinfile = preprocess_pkts(filename, max_len=args.max_len, normalized=False)
